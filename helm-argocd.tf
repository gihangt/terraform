resource "helm_release" "argocd" {
  ...
  values = [
    file("values/argocd-helm-values.yaml"),
    jsonencode({
      configs : {
        rbac : {
          "policy.csv" : var.admin_policy_csv
          "policy.dev.csv" : var.suffix == "dev" ? var.dev_arogcd_dev_policy_csv : var.dev_arogocd_prod_policy_csv
          "policy.int-dev.csv" : var.suffix == "prod" ? var.int_dev_argocd_prod_policy_csv : ""
        }
      }
    })
  ]
